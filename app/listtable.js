...
listTable.status = function(obj,uid,admin){
  var title = admin==null ? '确定要激活会员，将扣除你'+obj.attr('buymoney') :  '确定要激活会员，将结算奖金';
  var mess = admin==null ? '安全密码' : '管理密码';
  showhandle({
	...
	removetip("toggle");
    if(admin==null){
	   var url = get_path_url("?mod=member&act=vocational&type=ajax&repass="+$("#repass").val()+"&uid="+uid);
    }else{
    ...
    $.getJSON(url,function(res){
	  ...
	  if(res.error=='0'){
		...
		if(admin==null){
		  ...
		  obj.html('<p><em>激活时间：</em></p><p style="padding-left:5px;"><em></em>'+res.opentime+'</p>');
		}
	  }else{
...
listTable.statususer = function(uid){
  showhandle({
    html:'<table class="lotab"><tr><th><label>安全密码：</label></th><td><input id="repass" class="myinput repass" type="password"></td></tr></table>',
    width:320,
    height:136,
    id:'upgroup',
    title:'确定要激活会员，将扣除你'+$('#buymoney').val()
  },function(){
	$("#controlLoad").show();
	removetip("upgroup");
    $.getJSON(get_path_url("?mod=member&act=vocational&type=ajax&repass="+$("#repass").val()+"&username="+encodeURIComponent(username)),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
		hidebox('upgroup',true);
		Right('激活玩家账号成功',{},function(){
...
listTable.moneycheck = function(uid,message){
  showhandle({
    html:'<table class="lotab"><tr><th><label>安全密码：</label></th><td><input id="repass" class="myinput repass" type="password"></td></tr></table>',
    width:320,
    height:136,
	...
	$("#controlLoad").show();
	removetip("upgroup");
    $.getJSON(get_path_url("?mod=member&act=user&type=fuser&re=ajax&do=moneycheck&repass="+$("#repass").val()+"&uid="+uid),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
		hidebox('upgroup',true);
		Right('激活账号分红成功',{},function(){
          location.href = res.url;				
		});
	  }else{
		addtip("upgroup",res.error);
...
listTable.buytype = function(groupid){
  ...
  showhandle({
	...
	$("#controlLoad").show();
	removetip("upgroup");
    $.getJSON(get_path_url("?mod=member&act=vocational&type=buytype&re=ajax&repass="+$("#repass").val()+"&groupid="+groupid),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
		hidebox('upgroup',true);
		Right('业务激活成功',{},function(){
          location.href = res.url;				
		});
	  }else{
		addtip("upgroup",res.error);
...
listTable.upgroup = function(groupid){
  showhandle({
	...
	$("#controlLoad").show();
	removetip("upgroup");
    $.getJSON(get_path_url("?mod=member&act=vocational&type=upgroup&re=ajax&repass="+$("#repass").val()+"&groupid="+groupid),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
		hidebox('upgroup',true);
...
listTable.register = function(){
  showhandle({
    ...
    $.ajax({url:get_path_url("?mod=member&act=vocational&type=register&repass="+$("#repass").val()),
	  type:'POST',
      dataType:"json",
	  data:queryString,
      success:function(res){
...
listTable.customs = function(name,address){
  var par = "&name="+encodeURIComponent(name)+"&address="+encodeURIComponent(address);
  showhandle({
    html:'<table class="lotab"><tr><th><label>安全密码：</label></th><td><input id="repass" class="myinput repass" type="password"></td></tr></table>',
    width:320,
    height:140,
	...
	$("#controlLoad").show();
	removetip("customs");
    $.getJSON(get_path_url("?mod=member&act=vocational&type=customs&re=ajax"+par+"&repass="+$("#repass").val()),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
		hidebox('customs',true);
		Right('报单中心申请成功',{},function(){
          location.href = res.url;				
		});
	  }else{
		addtip("customs",res.error);
	  }							  
    });
  });
}

listTable.inmember = function(username,password,repass,referee,_referee,position,groupid){
  showhandle({
...
listTable.removeuser = function(id, cfm, opt){
  ...
  showhandle({
	...
	$("#controlLoad").show();
	removetip("_remove");
    $.getJSON(get_path_url("?mod=member&act=vocational&type=ajax&do=delete&repass="+$("#repass").val()+"&uid="+id),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
...
listTable.trading = function(obj,id,check){
  var title = check==1 ? '确定要购买该单吗' :  '确定已经收款吗？';
  showhandle({
    html:'<table class="lotab"><tr><th><label>安全密码：</label></th><td><input id="repass" class="myinput repass" type="password"></td></tr></table>',
    width:320,
    height:136,
	...
	$("#controlLoad").show();
	removetip("toggle");
    $.getJSON(get_path_url("?mod=member&act=trading&type=ajax&repass="+$("#repass").val()+"&id="+id),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
		hidebox('toggle',true);
		obj.html(res.html);
...
listTable.memberRemove = function(id, cfm, opt){
  ...
  showhandle({
	...
	$("#controlLoad").show();
	removetip("_remove");
    $.getJSON(listTable.url+'&do='+opt+'&id='+id+"&repass="+$("#repass").val(),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
		hidebox('_remove',true);
		Right(cfm+'成功',{},function(){
          location.href = res.url;				
		});
	  }else{
		addtip("_remove",res.error);
...
listTable.memberfrom = function(title,obj,url){
  showhandle({
    html:'<table class="lotab"><tr><th><label>安全密码：</label></th><td><input id="_repass" class="myinput repass" type="password"></td></tr></table>',
    width:320,
    height:138,
    id:'ajaxform',
    title:title
