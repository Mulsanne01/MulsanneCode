  ...
  <div class="right">
    <div class="opencard_main">
      <div class="track_title"> 
        <a href="<?=Purl(memberpre(1)); ?>" class="<?=!$_GET['typeid'] ? 'menushow' : 'menu'; ?>">所有公告</a> 
        <? if(is_array($this->getnewstype(10))) { foreach($this->getnewstype(10) as $val) { ?>        <a href="<?=Purl(memberpre(1).'&typeid='.$val['id']); ?>" class="<?=$_GET['typeid']==$val['id'] ? 'menushow' : 'menu'; ?>"><?=$val['typename']?></a> 
        <? } } ?>   
      </div>
      <? if($_GET['type']=='list') { ?>
      ...
      <div class="article">
        ...
        <p class="time">发布时间：<?=formattime($this->news['addtime']); ?> </p>
